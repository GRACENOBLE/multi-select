class MultiSelect {
  ...
  _template() {
    ...
    for (let i = 0; i < this.data.length; i++) {
      optionsHTML += `
 <div class="multi-selectoption${
   this.selectedValues.includes(this.data[i].value)
     ? " multi-selectselected"
     : ""
...
name="${this.name}[]" value="${value}">`
 ...
 <span class="multi-select-headerplaceholder">${this.placeholder}</span>
 </div>
      ...
      option.onclick = () => {
        ...
        if (!option.classList.contains("multi-select-selected")) {
          ...
          if (
            ...
            if (this.element.querySelector(".multi-select-headeroption")) {
              let opt = Array.from(
              ...
              opt.insertAdjacentHTML(
                "afterend",
                `<span
class="multi-select-header-option" datavalue="${option.dataset.value}">${
                  option.querySelector(".multi-select-optiontext").innerHTML
                }</span>`
              );
            } else {
...
class="multi-select-header-option" datavalue="${option.dataset.value}">${
          ...
          this.element.querySelector(".multiselect").insertAdjacentHTML(
            "afterbegin",
            `<input type="hidden"
name="${this.name}[]" value="${option.dataset.value}">`
          );
          this.data.filter(
          ...
          this.element
            .querySelectorAll(".multi-select-headeroption")
        ...
        if (
          ...
          if (this.element.querySelector(".multi-select-headeroption")) {
            this.element.querySelector(".multi-select-headeroption").remove();
          }
          headerElement.insertAdjacentHTML(
            "afterbegin",
            `<span
class="multi-select-header-option">${this.selectedValues.length}
selected</span>`
          );
        }
        if (!this.element.querySelector(".multi-select-header-option")) {
          headerElement.insertAdjacentHTML(
            "afterbegin",
            `<span
class="multi-select-header-placeholder">${this.placeholder}</span>`
          );
        } else if (
          this.element.querySelector(".multi-select-headerplaceholder")
        ) {
          this.element
            .querySelector(".multi-select-headerplaceholder")
            .remove();
        }
        if (this.options.max) {
          this.element.querySelector(".multi-select-headermax").innerHTML =
            this.selectedValues.length + "/" + this.options.max;
        }
        ...
        this.element
          .querySelectorAll(".multi-selectoption")
    ...
    headerElement.onclick = () =>
      headerElement.classList.toggle("multiselect-header-active");
    if (this.options.search === true || this.options.search === "true") {
        ...
        this.element
          ...
          .forEach((option) => {
            option.style.display =
              option
                .querySelector(".multi-selectoption-text")
    ...
    if (this.options.selectAll === true || this.options.selectAll === "true") {
      let selectAllButton = this.element.querySelector(".multi-selectall");
    ...
    if (this.options.listAll === true || this.options.listAll === "true") {
      this.element
        ...
        .forEach((option) => {
          if (option.classList.contains("multi-select-selected")) {
            this.element
              .querySelector(".multi-selectheader")
              .insertAdjacentHTML(
                "afterbegin",
                `<span class="multi-select-headeroption" data-value="${
                  option.dataset.value
                }">${
                  option.querySelector(".multiselect-option-text").innerHTML
                }</span>`
              );
          }
        });
      ...
      if (this.selectedValues.length > 0) {
        this.element
          ...
          .insertAdjacentHTML(
            "afterbegin",
            `<span class="multi-select-headeroption">${this.selectedValues.length} selected</span>`
          );
      }
    }
    if (this.element.querySelector(".multi-select-header-option")) {
      this.element.querySelector(".multi-select-headerplaceholder").remove();
    }
  }
  get selectedValues() {
